...
| 🧭 **질문 탐험대** | 조건 필터로 1–30장 무작위 뽑기 | 카테고리/유형 다중 선택 · 뽑은 뒤 모두 뒤집기·🎯 미션카드 |
| 🎰 **질문 로또** | 무작위 한 장만 등장, 다시 뽑기 | 연속 중복 방지 · 등장 애니메이션 · 클릭 플립 · 뽑은 뒤 🎯 미션카드 |
...
### 🧭 질문 탐험대
카테고리/유형 다중 선택 + '전체' 토글 · 1–30장 무작위 뽑기 · 풀이 부족하면 안내 모달.

> 🧹 **단순한 흐름**: 가운데 점선 박스에는 **‘몇 장 뽑을까요? → 뽑기!’** 한 가지 동작만 있어요. 카드를 뽑고 나면 그 아래에 **‘모두 뒤집기’** 와 **‘🎯 미션카드 뽑기’** 보조 도구가 나타나, 처음 화면이 깔끔하고 직관적이에요.
...
- **탐험대·로또에서 카드를 뽑은 뒤** 나타나는 **🎯 미션카드 뽑기** 버튼을 누르면, 작은 모달에 미션 카드 한 장이 무작위로 떠요. **다시 뽑기**로 계속 갱신할 수 있어요.
- "질문을 뽑은 다음에 미션을 더한다"는 흐름에 맞게, 미션 버튼은 **뽑기 전에는 숨겨 두고** 뽑은 뒤에만 보여 줘서 화면이 복잡해지지 않아요.
...
### 2026-06
- **🧭 질문 탐험대 UI 정돈**: 점선 박스에 뽑기/모두 뒤집기/미션 버튼이 한꺼번에 묶여 복잡하던 것을 단순화. 점선 박스는 **‘몇 장 → 뽑기!’** 한 동작만 담고, **‘모두 뒤집기’·‘🎯 미션카드 뽑기’** 는 카드를 뽑은 뒤에만 나타나는 보조 도구 바로 분리. 로또도 동일하게 미션 버튼을 뽑은 뒤 노출.
